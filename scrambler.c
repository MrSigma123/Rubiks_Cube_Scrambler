...
void scramblerTimer(void){
  ...
  int val1 = 0;
  int val2 = 0;
  int scrambleLength = 25; // use scrambleLength = 1; for test specific cube turns
  ...
  for(i = 0; i < scrambleLength; i++){
    if (i == 0)
      val1 = rand() % 18; // use val1 = x; for tests, where x is the value corresponding to the number assigned to the cube turn
    //val2 = rand() % 18;
    //printf("%d ", val1);
    switch (val1){
      case 0:
